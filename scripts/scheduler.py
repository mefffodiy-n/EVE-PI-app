...
Их шесть — статус сервера, рыночные цены, обновление ESI-токенов,
синхронизация скиллов и статуса колоний, скелет планет из SDE (Фаза 2
мультирегиональности, 21.09.2026). Этот скрипт запускает их с нужными
интервалами одним процессом, с журналом в файл (Фаза 6).
...
JOBS = [
    ...
    Job("Скелет планет из SDE", "refresh_sde", _refresh_sde, 360,
        "полный список регионов/систем/планет; SDE меняется резко реже "
        "рынка, дешёвая HEAD-проверка билда раз в 6 часов достаточно"),
]
