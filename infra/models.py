...
class Character(Base):
    ...
    # "dev" (никогда не проходит SSO) или esi-персонаж, вошедший ДО этой
    # правки и ни разу не заходивший с тех пор (мигрирует при следующем
    # входе). Внутри группы у ВСЕХ персонажей (включая основного — он
    # ссылается сам на себя) одно и то же значение — character_id
    # основного; от него детерминированно строится account_id
    # (f"char:{primary_character_id}"), одинаковый при входе с любого
    # устройства/браузера, а не случайный на сессию.
...
class Colony(Base):
    ...
    # проекция вперёд по образцу стороннего PI-инструмента с открытым
    # кодом, а не выдуманное «работает/не работает». None/[] — колония
    # ни разу не синхронизирована этим
