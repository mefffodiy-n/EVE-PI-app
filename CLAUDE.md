...
в ограничения. Данные о планетах хранятся в БД (таблицы `regions`/`planets`,
`infra/models.py` — с 21.09.2026, Фаза 1 мультирегиональности). Скелет
(констелляция/система/тип/радиус) с Фазы 2 (тоже 21.09.2026) загружен для
ВСЕХ регионов New Eden — `scripts/refresh_sde.py` наполняет его из
...
не нужен); в продакшене (с 20.09.2026) — MariaDB. Postgres поддерживается
так же полно и была прод-СУБД до этой даты (`docs/ROADMAP.md`, «Переход на
Postgres», «Миграция прода на MariaDB») — обе решают задачу раздела 5:
multi-tenant и конкурентная запись из воркеров. Переключение между любыми
поддерживаемыми движками — только сменой URL и `alembic upgrade head`;
модели те же (`deploy/README.md`, раздел 3 — что проверено для каждого).
...
scripts/          всё, что ходит в сеть или готовит данные
  ...
  refresh_sde.py           скелет planets по всем регионам из SDE CCP (Фаза 2)
