...
class PlanRequest:
    ...
    # Одна «линия» = один полный шаблон целевого продукта. С 18.09.2026
    # подключено к интерфейсу (по прямому запросу пользователя —
    # продублировать выбранную цепочку, а не подбирать второй-третий
    # продукт вручную). НЕ участвует в direct_p2 (приостановлен, см.
    # _plan_direct_p2()/_place_direct_p2() ниже) — там сайзинг жёстко
    # «выпуск одного обычного завода», без учёта числа линий; честно
    # зафиксировано как известный, но не устраняемый сейчас пробел
    # (docs/ROADMAP.md).
